...
pacman::p_load(lme4, lmerTest, broom.mixed, sjPlot)
...
# モデル1: 介入群効果（ランダム切片：被験者）
glmm1 <- glmer(violation ~ category + (1 | participant.id_in_session),
               data = glmm_data,
...
glmm2 <- glmer(violation ~ category * DQS + (1 | participant.id_in_session),
               data = glmm_data,
               family = binomial(link = "logit"),
               control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)))
