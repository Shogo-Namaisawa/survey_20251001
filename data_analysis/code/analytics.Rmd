...
概要：本研究は，クラウドソーシングによるアンケート調査において，努力最小化（satisfice）により生じる不真面目回答の抑制を目的とし，ナッジ介入の効果を検証するものである．従来から用いられてきた事前申告や利得強調，損失回避型の手法に加え，大規模言語モデル（LLM）により生成されたナッジメッセージが回答品質の向上に寄与するかを，アンケート回答者をランダムに振り分けて比較する平均処置効果（ATE）によって検証した．その結果，利得強調や損失回避による介入で抑制させる効果がみられたものの，すべての介入群で統計的に有意な抑制効果は見られなかった．このことから，不真面目回答に対する効果は存在したとしても小さな効果量であり，本研究のサンプル規模（各群約180）では統計的に有意な差として検出するには不十分であったことが示唆された．
...
5つのデータセットを読み込み、基本データセットから必要なカラムを抽出して一つの新しいデータフレームを作成

ダミー回帰分析用にone-hot encodingを実施する
...
必要な列のみ抽出し、category列を追加（以降using_llmまで同じ）
...
cat("=== Or_DQS の分布 ===\n
    真面目回答者:", or_dqs_table["0"], "\n
    不真面目回答者:", or_dqs_table["1"], "\n
    途中離脱者:", or_dqs_table[is.na(names(or_dqs_table))], "\n
    総計:", sum(or_dqs_table))
...
# データ除外後の情報確認
cat("\n=== 欠損値データ除外後の情報 ===\n", 
    "除外後の総行数:", nrow(dummy_all_data), "\n", 
    "除外された行数:", nrow(all_data) - nrow(dummy_all_data))
```
---
...
# ダミー変数の分布確認
cat("\n=== ダミー変数のデータ数の確認 ===\n", 
    "basic:", sum(dummy_all_data$basic), "\n", 
    "gain_emphasis:", sum(dummy_all_data$gain_emphasis), "\n", 
    "loss_aversion:", sum(dummy_all_data$loss_aversion), "\n", 
    "pre_commitment:", sum(dummy_all_data$pre_commitment), "\n", 
    "using_llm:", sum(dummy_all_data$using_llm))
```
...
cat("\n=== ダミー回帰用データのDQS分布確認 ===\n", 
    "DQS_1 - 真面目:", dummy_dqs1_table["0"], ", 不真面目:", dummy_dqs1_table["1"], ", NA:", dummy_dqs1_table[is.na(names(dummy_dqs1_table))], "\n")
...
## GLMM（ロジスティック混合モデル）を用いたlong-format解析
この解析では、各被験者を3つの試行（DQS_1, DQS_2, DQS_3）を持つ長い形式（long format）に変換し、被験者ごとのランダム切片を含むロジスティック混合モデルで解析します。介入効果は統制群を基準として評価します。

```{r glmm-long-format, message=FALSE, warning=FALSE}
# 追加パッケージ（インストールされていない場合は事前にインストールしてください）
pacman::p_load(lme4, lmerTest, broom.mixed, sjPlot, glmmTMB)

# データの整形（long format）
glmm_data <- dummy_all_data %>%
  select(participant.id_in_session, category, DQS_1, DQS_2, DQS_3) %>%
  pivot_longer(cols = starts_with("DQS_"), names_to = "DQS", values_to = "violation") %>%
  mutate(
    DQS = factor(DQS, levels = c("DQS_1", "DQS_2", "DQS_3")),
    category = factor(category, levels = c("統制群", "利得強調", "損失回避", "事前申告", "LLM活用")),
    violation = as.integer(violation)
  )

# 確認（簡単なクロス集計）
print(table(glmm_data$category, glmm_data$DQS))
print(table(glmm_data$DQS, glmm_data$violation))

## モデル適合上の確認: データに極端な偏り（完全分離やカテゴリ内ゼロ）がないか確認します
cat("\n=== 被験者ごとの違反数サマリ（最小・最大・中央値等） ===\n")
print(summary(as.integer(table(glmm_data$participant.id_in_session))))
cat("\n=== 介入群×違反の頻度 ===\n")
print(table(glmm_data$category, glmm_data$violation))

# モデル1: まずは glmer を最適化オプションを変更して試行
glmm1 <- tryCatch(
  glmer(violation ~ category + (1 | participant.id_in_session),
        data = glmm_data,
        family = binomial(link = "logit"),
        control = glmerControl(optimizer = "nloptwrap", calc.derivs = FALSE, optCtrl = list(maxfun = 2e5))),
  warning = function(w) { w; invokeRestart("muffleWarning") },
  error = function(e) e
)

if(inherits(glmm1, "error")){
  cat("glmer failed: switching to glmmTMB for more robust fitting.\n")
  glmm1_tmb <- glmmTMB(violation ~ category + (1 | participant.id_in_session),
                       data = glmm_data,
                       family = binomial())
  print(summary(glmm1_tmb))
  tidy_glmm1 <- broom.mixed::tidy(glmm1_tmb, effects = "fixed", conf.int = TRUE, exponentiate = TRUE)
} else {
  # glmer 成功（又は警告を抑えた）場合は結果を表示
  print(summary(glmm1))
  tidy_glmm1 <- broom.mixed::tidy(glmm1, effects = "fixed", conf.int = TRUE, conf.level = 0.95, exponentiate = TRUE)
}

# 表示: tidy のデフォルト出力をそのまま表示
tidy_glmm1 %>%
  select(term, estimate, conf.low, conf.high, p.value) %>%
  kable(col.names = c("項目", "オッズ比", "95%下限", "95%上限", "p値"), caption = "GLMM: 介入群のオッズ比 (参照: 統制群)")

# モデル2: DQS指標との交互作用を含めたモデル（介入群ごとにDQSで効果が異なるか）
# モデル2: 交互作用モデル。glmer を試し、失敗したら glmmTMB にフォールバック
glmm2 <- tryCatch(
  glmer(violation ~ category * DQS + (1 | participant.id_in_session),
        data = glmm_data,
        family = binomial(link = "logit"),
        control = glmerControl(optimizer = "nloptwrap", calc.derivs = FALSE, optCtrl = list(maxfun = 2e5))),
  warning = function(w) { w; invokeRestart("muffleWarning") },
...
if(inherits(glmm2, "error")){
  cat("glmer (interaction) failed: switching to glmmTMB.\n")
  glmm2_tmb <- glmmTMB(violation ~ category * DQS + (1 | participant.id_in_session),
                       data = glmm_data,
                       family = binomial())
  print(summary(glmm2_tmb))
  tidy_glmm2 <- broom.mixed::tidy(glmm2_tmb, effects = "fixed", conf.int = TRUE, exponentiate = TRUE)
} else {
  print(summary(glmm2))
  tidy_glmm2 <- broom.mixed::tidy(glmm2, effects = "fixed", conf.int = TRUE, exponentiate = TRUE)
}

tidy_glmm2 %>%
  filter(str_detect(term, "category")) %>%
  select(term, estimate, conf.low, conf.high, p.value) %>%
  kable(col.names = c("項目", "オッズ比", "95%下限", "95%上限", "p値"), caption = "GLMM（交互作用あり）：介入群×DQS のオッズ比")

# モデル比較（簡易）
data.frame(model = c("glmm1", "glmm2"), AIC = c(AIC(glmm1), AIC(glmm2)), BIC = c(BIC(glmm1), BIC(glmm2))) %>%
  kable(col.names = c("モデル", "AIC", "BIC"))
```
